...
namespace ACRCloudRecognitionTest
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            config.Add("host", "identify-ap-southeast-1.acrcloud.com");
            config.Add("access_key", "576620619f8163f507b4114d41142899");
            config.Add("access_secret", "gD8qnAplhNNchhEis5fMu771YUWyCA8Cz3mLxYh5");
            config.Add("timeout", 10); // seconds
            config.Add("rec_type", ACRCloudRecognizer.RECOGNIZER_TYPE.acr_rec_type_audio);

            ACRCloudRecognizer re = new ACRCloudRecognizer(config);

            string tempFilePath = Path.Combine(Path.GetTempPath(), "result.wav");
            string result = re.RecognizeByFile(tempFilePath, 0);
            ...
            try
            {
                var music = json["metadata"]["music"][0];
            ...
            else
            {
                using (NotifyIcon notifyIcon = new NotifyIcon())
                {
                    ...
                    notifyIcon.Text = "Crop to Search"; 
                    notifyIcon.BalloonTipTitle = "Crop to Search";
                    notifyIcon.BalloonTipText = "Song not found";
